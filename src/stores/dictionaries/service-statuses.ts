...
import { TSelectionModel } from "@/models/dictionaries/selection";
import { http } from "@/utils/http";

export const useServiceStatusesStore = create(
  persist<{
    date: Date | null;
    loading: boolean;
    error: string | null;
    total: number | null;
    items: TSelectionModel[] | null;
    getItems: (filters?: TModelFilters) => TSelectionModel[] | null;
    ...
    (set, get) => ({
      ...
      fetchItems: async (filters?: TModelFilters) => {
        ...
        try {
          ...
          const response = await http("/ws/selection/concept.order.status.type.select", {
            method: "POST",
    ...
    { name: "serviceStatusesDictionary" }
  )
);
